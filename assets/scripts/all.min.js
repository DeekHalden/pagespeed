...
!function(n,t,a){n.fn.responsiveSlides=function(e){var s=n.extend({auto:!0,speed:500,timeout:4e3,pager:!1,nav:!1,random:!1,pause:!1,pauseControls:!0,prevText:"Previous",nextText:"Next",maxwidth:"",navContainer:"",manualControls:"",namespace:"main_slider__slides",before:n.noop,after:n.noop},e);return this.each(function(){a++;var o,i,r,l,u,c,f=n(this),d=0,p=f.children(),h=p.length,v=parseFloat(s.speed),m=parseFloat(s.timeout),C=parseFloat(s.maxwidth),x=s.namespace,y=x+a,b=x+"_nav "+y+"_nav",g=x+"_here",_=y+"_on",w=y+"_s",I=n("<ul class='"+x+"_tabs "+y+"_tabs' />"),q={"float":"left",position:"relative",opacity:1,zIndex:2},z={"float":"none",position:"absolute",opacity:0,zIndex:1},T=function(){var n=document.body||document.documentElement,t=n.style,a="transition";if("string"==typeof t[a])return!0;o=["Moz","Webkit","Khtml","O","ms"],a=a.charAt(0).toUpperCase()+a.substr(1);var e;for(e=0;e<o.length;e++)if("string"==typeof t[o[e]+a])return!0;return!1}(),k=function(t){s.before(t),T?(p.removeClass(_).css(z).eq(t).addClass(_).css(q),d=t,setTimeout(function(){s.after(t)},v)):p.stop().fadeOut(v,function(){n(this).removeClass(_).css(z).css("opacity",1)}).eq(t).fadeIn(v,function(){n(this).addClass(_).css(q),s.after(t),d=t})};if(s.random&&(p.sort(function(){return Math.round(Math.random())-.5}),f.empty().append(p)),p.each(function(n){this.id=w+n}),f.addClass(x+" "+y),e&&e.maxwidth&&f.css("max-width",C),p.hide().css(z).eq(0).addClass(_).css(q).show(),T&&p.show().css({"-webkit-transition":"opacity "+v+"ms ease-in-out","-moz-transition":"opacity "+v+"ms ease-in-out","-o-transition":"opacity "+v+"ms ease-in-out",transition:"opacity "+v+"ms ease-in-out"}),p.length>1){if(v+100>m)return;if(s.pager&&!s.manualControls){var F=[];p.each(function(n){var t=n+1;F+="<li><a href='#' class='"+w+t+"'>"+t+"</a></li>"}),I.append(F),e.navContainer?n(s.navContainer).append(I):f.after(I)}if(s.manualControls&&(I=n(s.manualControls),I.addClass(x+"_tabs "+y+"_tabs")),(s.pager||s.manualControls)&&I.find("li").each(function(t){n(this).addClass(w+(t+1))}),(s.pager||s.manualControls)&&(c=I.find("a"),i=function(n){c.closest("li").removeClass(g).eq(n).addClass(g)}),s.auto&&(r=function(){u=setInterval(function(){p.stop(!0,!0);var n=h>d+1?d+1:0;(s.pager||s.manualControls)&&i(n),k(n)},m)})(),l=function(){s.auto&&(clearInterval(u),r())},s.pause&&f.hover(function(){clearInterval(u)},function(){l()}),(s.pager||s.manualControls)&&(c.bind("click",function(t){t.preventDefault(),s.pauseControls||l();var a=c.index(this);d===a||n("."+_).queue("fx").length||(i(a),k(a))}).eq(0).closest("li").addClass(g),s.pauseControls&&c.hover(function(){clearInterval(u)},function(){l()})),s.nav){var M="<a href='#' class='"+b+" prev'>"+s.prevText+"</a><a href='#' class='"+b+" next'>"+s.nextText+"</a>";e.navContainer?n(s.navContainer).append(M):f.after(M);var D=n("."+y+"_nav"),O=D.filter(".prev");D.bind("click",function(t){t.preventDefault();var a=n("."+_);if(!a.queue("fx").length){var e=p.index(a),o=e-1,r=h>e+1?d+1:0;k(n(this)[0]===O[0]?o:r),(s.pager||s.manualControls)&&i(n(this)[0]===O[0]?o:r),s.pauseControls||l()}}),s.pauseControls&&D.hover(function(){clearInterval(u)},function(){l()})}}if("undefined"==typeof document.body.style.maxWidth&&e.maxwidth){var W=function(){f.css("width","100%"),f.width()>C&&f.css("width",C)};W(),n(t).bind("resize",function(){W()})}})}}(jQuery,this,0);
function initMap(){function e(){o.setOptions({scrollwheel:!0})}function n(){o.setOptions({scrollwheel:!1})}var t={lat:49.9719322,lng:36.1608323},o=new google.maps.Map(document.getElementById("map"),{zoom:18,center:t,disableDefaultUI:!0,mapTypeId:google.maps.MapTypeId.ROADMAP});new google.maps.Marker({position:t,map:o,animation:google.maps.Animation.DROP,title:"Hello World!",icon:"../assets/images/pin_large.png"});google.maps.event.addListener(o,"mousedown",function(){e()}),$("body").on("mousedown",function(e){var t=$(e.target).parents(".contacts_map")[0].length>0;t||n()}),$(window).scroll(function(){n()})}$(function(){var e=document.getElementById("map"),n=($("#map").css("width"),$("#map").css("height"),0);$("#enter-full-screen").click(function(){if(0==n){$(".contacts__map").css("height","500px");setInterval(function(){google.maps.event.trigger(e,"resize"),clearInterval()},1);n++}else{$(".contacts__map").css("height","200px");setInterval(function(){google.maps.event.trigger(e,"resize"),clearInterval()},1);n--}})});
function toggleMenu(){navButton.classList.toggle("active"),collapse()}function collapse(){var e=document.getElementById("nav");"header__navigation"===e.className?e.className+=" responsive":e.className="header__navigation"}var navButton=document.getElementsByClassName("nav_button")[0];navButton.addEventListener("click",toggleMenu);
$(document).ready(function(){$("#equipment__slider").lightSlider({gallery:!0,item:1,loop:!0,slideMargin:0,thumbItem:3}),$("#equipment__additional_slider").lightSlider({item:4,loop:!0,slideMove:2,speed:600,responsive:[{breakpoint:800,settings:{item:3,slideMove:1,slideMargin:6}},{breakpoint:480,settings:{item:1,slideMove:1}}]})});
tter-webfont-loading-with-localstorage-and-woff2.js
...
for(var path=window.location.pathname,page=path.split("/").pop(),els=document.getElementsByClassName("header__navigation--element"),i=0;i<els.length;i++){var a=els[i].childNodes[0].href;els[i].childNodes[0].classList.remove("active"),a.substring(a.lastIndexOf("/"))=="/"+page&&(console.log(els[i].childNodes[0].href),els[i].childNodes[0].classList.add("active"))}
$(function(){$(".main-slider").responsiveSlides({auto:!1,pager:!1,nav:!0,speed:500}),$(".additional-slider").responsiveSlides({auto:!1,pager:!1,nav:!0,speed:500})});
$(function(){$('ul a[href*="#"]:not([href="#"])').click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var t=$(this.hash);if(t=t.length?t:$("[name="+this.hash.slice(1)+"]"),t.length)return $("html, body").animate({scrollTop:t.offset().top},2e3),!1}})});